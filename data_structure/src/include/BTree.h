...
{
	BTNode<T> *St[MAX_SIZE], *p = 0;       //St数组作为顺序栈
	int top = -1, k = 1, j = 0;            //top为栈顶指针
	T ch;
	b = 0;                             //初始二叉树为空
	ch = str[j];
...
{
	TBTNode<T> *St[MAX_SIZE], *p = 0;       //St数组作为顺序栈
	int top = -1, k = 1, j = 0;            //top为栈顶指针
	T ch;
	b = 0;                             //初始二叉树为空
	ch = str[j];
...
BTNode<char> *CreateBT1(char const* pre, char const* inner, int n)
{
	...
	b->data = *pre;
	for (p = (char *)inner; p < inner + n; p++) {           //在中序序列中找等于*ppos的位置k
		if (*p == *pre) {                    //pre指向根结点
			break;                             //在in中找到后退出循环
		}
	}
	k = int(p - inner);                               //确定根结点在in中的位置
	b->lchild = CreateBT1(pre + 1, inner, k);        //递归构造左子树
